...
We can also add new functionality to classes with the +mixin()+ method (runtime) or with the +@Mixin+ annotation (compile-time). We specify the class as an argument to the method or annotation. All public methods from the class are added to our own class.

[source,groovy]
----
package org.gr8conf.groovy

import org.gr8conf.java.JavaTalk
